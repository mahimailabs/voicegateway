...
def get_or_freeze_guardrail_policy_snapshot(
    ...
    This is called by ``InstrumentedLLM.chat`` on the first LLM request
    in a session. Later calls in the same context reuse the snapshot so
    dashboard policy edits affect only subsequent sessions.
    """
