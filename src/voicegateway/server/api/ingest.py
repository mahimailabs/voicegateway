...
def _record_from_payload(raw: dict[str, Any]) -> RequestRecord | None:
    ...
    Billing note: any ``rated_price_usd`` / ``rate_rule`` on the payload is
    persisted as-is. Agents rate at cost pass-through (no card client-side),
    so fleet rows arrive unrated. Re-rating ingested rows against the
    collector's own card is a follow-up; today only requests generated by the
    collector's own gateway are rated. See docs/architecture/rating.md.
    """
