...
- [`voicegw status`](/docs/cli/status): verify the wizard wrote a
  working config.
- [`voicegw doctor`](/docs/cli/status): numbered punch list when
  something is off.
- [`voicegw start`](/docs/cli/serve): start the daemon manually if
  you skipped the install step.
- [`voicegw dashboard`](/docs/cli/dashboard): open the dashboard URL
  in your browser.
