...
<Warning>
`plan` is a real scope field, settable when a rule is created (CLI, HTTP, MCP, or dashboard), but nothing on the live write path attaches a plan to a request being rated: `CostTracker._rate()` calls `rating.price(..., tenant=current_tenant())` and never passes `plan`, so resolution always runs with `plan=None`. A plan-scoped rule can be created and will show up in `voicegw prices ls`, but it cannot currently match a real request. `tenant` has no such gap: it's read from the same `current_tenant()` `ContextVar` `attach(tenant_id=...)` sets.
</Warning>
