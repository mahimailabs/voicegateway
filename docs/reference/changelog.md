...
- **Resolver does not yet read DB-managed per-project rows.** `vg_add_provider` persists rows to `managed_providers` with the `project` column populated and Fernet-encrypts the key. The inference resolver currently consults only YAML-defined `projects.<id>.providers.<name>` blocks at request time. End-to-end DB-driven per-project resolution lands in v0.0.6+ via either an async resolver or a flatten-into-yaml step in `ConfigManager.load_merged`.
